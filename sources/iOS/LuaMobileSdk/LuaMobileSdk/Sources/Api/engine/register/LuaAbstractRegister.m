...
@implementation LuaAbstractRegister {
    NSMutableArray<LuaAbstractBridge*>* mBridgesHolder;
}
...
- (instancetype)initWithEngine:(LuaScriptEngine*)engine {
    if (self = [super init]) {
        ...
        mBridgesHolder = [NSMutableArray new];
    }
...
- (void)destroy {
    for (LuaAbstractBridge* bridge in mBridgesHolder) {
        [bridge destroy];
    }

    [mBridgesHolder removeAllObjects];
}

- (void)holdBridge:(LuaAbstractBridge*)bridge {
    [mBridgesHolder addObject:bridge];
}
